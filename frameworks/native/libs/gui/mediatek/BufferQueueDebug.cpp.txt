...
namespace android {
// -----------------------------------------------------------------------------
static status_t drawDebugLineToGraphicBuffer(
...
#else
    // Because compile option includes check of unused-variable, so these variables caused
    // compile error, and therefore we have to ingore them.
    (void)gb;
    (void)cnt;
    (void)val;
#endif
...
void BufferQueueDebug::onConstructor(
    ...
    mIsInGuiExt = (mConsumerName.find("GuiExt_") != -1);
...
void BufferQueueDebug::onSetConsumerName(const String8& consumerName) {
    ...
    // process BQ created in GuiExtService
    bool isInGuiExt = (mConsumerName.find("GuiExt_") != -1);
    if (mIsInGuiExt != isInGuiExt) {
        mIsInGuiExt = isInGuiExt;
        mDump = new BufferQueueDump(mIsInGuiExt ?
                BufferQueueDump::TRACK_PRODUCER : BufferQueueDump::TRACK_CONSUMER, mBq.unsafe_get());
        if (mDump == NULL) {
            BQ_LOGE("new BufferQueueDump() failed in setConsumerName()");
        }

        if (mIsInGuiExt) {
            // Do not monitor BQ in GuiExtService
            BufferQueueMonitor::getInstance().unmonitor(mBq);
        } else {
            // Monitor BQ does not in GuiExtService
            BufferQueueMonitor::getInstance().monitor(mBq);
        }
    }
...
void BufferQueueDebug::setIonInfo(const sp<GraphicBuffer>& gb) {
    ...
    if (gb->handle != NULL) {
        ...
        if ((gb->usage & GRALLOC_USAGE_HW_COMPOSER) &&
            (mScenarioLayerType == GRALLOC_EXTRA_BIT2_LAYER_NAV)) {
