...
void AudioMixer::process__genericNoResampling(state_t* state)
{
    ...
    while (e0) {
        ...
        do {
            ...
            MixerDumpPcm(gaf_mixer_end_pcm, gaf_mixer_end_propty, gettid(),
...
#endif
        MixerDumpPcm(gaf_mixer_end_pcm, gaf_mixer_end_propty, gettid(), (int)out, out,
...
bool AudioMixer::track_t::doPostProcessing(void *buffer, audio_format_t format, size_t frameCount)
{
    ...
    if ((FCC_2 == mMixerChannelCount) &&
        (SurroundMix_enable || DRC_enable || (mSteroToMono == BLOUD_S2M_MODE_ST2MO2ST))) {
...
{
    DRC_ALOGD("DoStereoMonoConvert start mSteroToMono = %d, buffer %p, byte %d", mSteroToMono, buffer, byte);
...
#else
template <int MIXTYPE>
void AudioMixer::track_t::DoStereoMonoProcessing(void *buffer __unused,
    audio_format_t process_format __unused, size_t frameCount __unused, int32_t process_channel __unused)
{
}
...
template <int MIXTYPE, typename TO>
bool AudioMixer::track_t::DoStereoMonoConvert(void *buffer __unused, size_t byte __unused)
{
    return true;
}
#endif
