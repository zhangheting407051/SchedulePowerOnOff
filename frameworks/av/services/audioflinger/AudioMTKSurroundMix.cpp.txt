...
AudioMTKSurroundDownMix::AudioMTKSurroundDownMix() :
    IsBesSurroundInitialized(0), mTempBuf(NULL), mBesSurround(NULL), mDevice(0),
    mBesSurround_enable(0), mDownmix_enable(0), mBesSurroundMode(0), mDownmixHandle(NULL)
{
    ...
    mChannelMask = 0;
    mSampleRate = 0;
    memset(&mDownmixConfig, 0, sizeof(mDownmixConfig));
}

status_t AudioMTKSurroundDownMix::DownMix_Init(void* trackName, int sessionID, int channelMask, int sampleRate)
{
    ...
    if (EffectCreate(&(dwnmDesc.uuid),
       ...
       ALOGE("prepareTrackForDownmix(%p) fails: error creating downmixer effect", trackName);
       goto DownMixInitFail;
    }
    ...
    {// scope for local variables that are not used in goto label "noDownmixForActiveTrack"
        ...
        if ((status != 0) || (cmdStatus != 0)) {
            ALOGE("error %d while configuring downmixer for track %p", status, trackName);
            goto DownMixInitFail;
        }
        ...
        status = (*mDownmixHandle)->command(mDownmixHandle,
                ...
                &replySize /*replySize*/, &cmdStatus /*pReplyData*/);
        if ((status != 0) || (cmdStatus != 0)) {
            ALOGE("error %d while enabling downmixer for track %p", status, trackName);
            goto DownMixInitFail;
        }
        ...
        if ((status != 0) || (cmdStatus != 0)) {
            ALOGE("error %d while setting downmix type for track %p", status, trackName);
            goto DownMixInitFail;
        } else {
            ALOGV("downmix type set to %d for track %p", (int) downmixType, trackName);
        }
    }
...
status_t AudioMTKSurroundDownMix::Init(void* trackName, int sessionID, int channelMask, int sampleRate )
{
...
status_t AudioMTKSurroundDownMix:: process(int32_t* inBuffer, int32_t* ouBuffer, int32_t numFrames)
{
    ...
    MixerDumpPcm(gaf_mixer_before_downmix_pcm, gaf_mixer_downmix_propty, gettid(),
        static_cast<int>(reinterpret_cast<uintptr_t>(this)), (void*)inBuffer, popcount(mChannelMask) * 4 * numFrames,
        mDownmixConfig.inputCfg.format, mDownmixConfig.inputCfg.samplingRate, mDownmixConfig.inputCfg.channels );

    ALOGV("dump%d, channel count %d, numFrameCount %d", popcount(mChannelMask) * 4 * numFrames,popcount(mChannelMask),
        numFrames);
    ...
    if (mDownmix_enable || (!mDownmix_enable && mDownmixRamp == RAMP_DOWN)) {
        ...
        if (mDownmixHandle ==NULL) {
            ...
            ///////////////////////////////////////////////////////////////
            if (mSurroundRamp == RAMP_DOWN && IsBesSurroundInitialized) {
            ...
            if (mDownmix_enable && ((mBesSurround_enable && mDownmixRamp != RAMP_DOWN ) ||
                (!mBesSurround_enable && mSurroundRamp == RAMP_DOWN))) {
                if (mTempBuf == NULL) {
            ...
            MixerDumpPcm(gaf_mixer_after_downmix_pcm, gaf_mixer_downmix_propty, gettid(),
                static_cast<int>(reinterpret_cast<uintptr_t>(this)), (void*)mDownmixConfig.outputCfg.buffer.s32,
                popcount(mDownmixConfig.outputCfg.channels) * sizeof(int) * numFrames,
    ...
    if (IsBesSurroundInitialized != 0) {
        ...
        MixerDumpPcm(gaf_mixer_after_downmix_pcm, gaf_mixer_downmix_propty, gettid(),
            static_cast<int>(reinterpret_cast<uintptr_t>(this)), (void*)ouBuffer, 2 * 4 * numFrames,
            mDownmixConfig.outputCfg.format, mDownmixConfig.outputCfg.samplingRate, mDownmixConfig.outputCfg.channels);
        return OK;
    }
    return -1;
...
status_t AudioMTKSurroundDownMix:: SetBesSurroundOnOFF(int value)
{
    ...
    //value = 1;
    ALOGV("value %d mBesSurround_enable %d", value,mBesSurround_enable);
...
status_t AudioMTKSurroundDownMix:: SetBesSurroundMode(uint32_t value)
{
    ALOGV("SetBesSurroundMode value %d", value);
    if (value != 0 && value != 1) {
        return BAD_VALUE;
    }
    if (IsBesSurroundInitialized == 0) {
...
status_t AudioMTKSurroundDownMix::Qurey_Channel_Support(unsigned int channel_mask)
{
    ...
    if (((mDevice & (AUDIO_DEVICE_OUT_EARPIECE |
                     ...
                     AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES |
                     AUDIO_DEVICE_OUT_ANLG_DOCK_HEADSET |
                     AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET)) == 0) &&
...
status_t AudioMTKSurroundDownMix::BesSurround::BesSurround_info_initialize(MTK_VS_INFO *vs_info,
    int channel_mask, int sample_rate, int bits_per_sample, uint32_t device)
{
    if (vs_info == NULL) {
        ALOGD("vs_info allocate buffer fail!\n");
        return NO_INIT;
    }
    ALOGD("BesSurround_info_initialize channel_mask 0x%x, sample_rate %d,  bits_per_sample %d device %d",
        channel_mask, sample_rate, bits_per_sample, device);
...
status_t AudioMTKSurroundDownMix::BesSurround::BesSurround_reinit(int channelMask, int sampleRate)
{
    ...
    ALOGD("mBesSurround_info (%p) param(%p) mBesSurround_info->param.debug %d\n",
...
status_t AudioMTKSurroundDownMix::BesSurround::BesSurround_process(int32_t* inBuffer, int32_t* ouBuffer,
    int32_t numFrames)
{
    status_t res = OK;
...
status_t AudioMTKSurroundDownMix::BesSurround::BesSurround_SetMode(uint32_t value)
{
    status_t res = OK;
    if (value != 0 && value != 1) {
        return BAD_VALUE;
