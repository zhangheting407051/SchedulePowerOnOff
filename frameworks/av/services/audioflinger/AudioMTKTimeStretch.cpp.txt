...
AudioMTKTimeStretch::AudioMTKTimeStretch(int ratio, int inChannelCount,
    int sampleRate, int framecount __unused)
{
    initParameters();
...
void AudioMTKTimeStretch::initParameters()
{
    memset(&mBTS_RTParam, 0, sizeof(mBTS_RTParam));
    mpHandle = NULL;
...
size_t AudioMTKTimeStretch::InternalBufferSpace()
{
...
int AudioMTKTimeStretch::process(short* InputBuffer, short* OutputBuffer, int* inputByte, int* outputByte)
{
    ...
    case TS_STATE_SILENCE:
        {
            ALOGD("mInBuf_smpl_cnt %d", mInBuf_smpl_cnt);
            ALOGD("TS_STATE_SILENCE *inputByte %d,*outputByte %d,mFraction_remain %d",
                *inputByte, *outputByte, mFraction_remain);
            ...
            *outputByte = *outputByte < ((*inputByte) * mBTS_RTParam.TS_Ratio / 100)
                ? (*outputByte) : ((*inputByte) * mBTS_RTParam.TS_Ratio / 100);
            ...
            *inputByte = *inputByte - ((*outputByte + mFraction_remain) / ratio);
            ALOGD("after memset 0, *inputByte %d, *outputByte %d,mFraction_remain %d",
                *inputByte, *outputByte, mFraction_remain);
            //mRampFlag = RAMP_NORMAL;
        }
        break;
    case TS_STATE_RAMP_UP:
        ALOGV("mBTS_RTParam.TS_Ratio %d", mBTS_RTParam.TS_Ratio);
    ...
    if (mBTS_RTParam.TS_Ratio <= 400) {
        ...
        ALOGV("* input Buffer %p, *OutputBuffer %p , * inputByte %d, outputByte %d",
            InputBuffer, OutputBuffer, *inputByte, *outputByte);
...
int AudioMTKTimeStretch::setParameters(int *ratio)
{
    ...
    if (mProcessState == TS_STATE_INIT || mProcessState == TS_STATE_NORMAL) {
        ...
        if (*ratio >= 100 && (*ratio <= 400)) {
            mProcessState = TS_STATE_RAMP_UP;
            ALOGD("silence to ramp up");
        } else if (mProcessState == TS_STATE_RAMP_UP) {
            if (*ratio != mRatio_Cache) {
               // target ratio is changed during ramp down and ramp up
               // switch ratio here
               ALOGV("previous target  mRatio_Cache %d, new %d", mRatio_Cache, *ratio);
