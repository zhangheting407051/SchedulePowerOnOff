...
{
    ALOGV("readStateFromFile");
    int fd = open(HEADSET_STATE_PATH, O_RDONLY, 0);
    if(fd < 0)
    {
        ALOGD("Fail to open file %s", HEADSET_STATE_PATH);
        return -1;
    }
    char buf[1];
    if (read(fd, buf, 1) == -1){
        close(fd);
        ALOGD("Can't read %s", HEADSET_STATE_PATH);
        return -1;
    }
    close(fd);
    return buf[0];
...
bool  HeadsetDetect::headsetConnect(int stateVal)
{
     if(WIRE_HEADSET==stateVal){
        ALOGD("headset pluged in");
        return true;
    }else if(BIT_HEADSET_NO_MIC==stateVal) {
        ALOGD("headphone pluged in");
        return true;
    }else{
        ALOGD("pluged out");
        return false;
...
int HeadsetDetect::socketInit()
{
    ...
    iResult = setsockopt(s, SOL_SOCKET, SO_RCVBUFFORCE, &sz, sizeof(sz));
    if(iResult < 0) {
        ALOGV("iResult = %d, but don't do anything", iResult);
    }
