...
NotifySender::NotifySender()
    : Thread(false)
{
    SLOGD("NotifySender construct");
}

NotifySender::~NotifySender()
{
    SLOGD("NotifySender deconstruct in");
    ...
    requestExitAndWait();
    clear();
    SLOGD("NotifySender deconstruct out");
}
...
bool NotifySender::threadLoop()
{
    while (!exitPending())
    {
        NotifyMessage *p = 0;
        {
            ...
            p = *i;
            mMsgQueue.erase(i);
            SLOGV("messgesize=%d", mMsgQueue.size());
        }
...
{
    SLOGD("sendMessage::owner(%p),msg(%d)", owner.get(), msg);
    Mutex::Autolock _l(mLock);
