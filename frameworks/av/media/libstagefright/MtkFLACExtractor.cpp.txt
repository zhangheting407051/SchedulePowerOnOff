...
#define LOG_TAG "MtkFLACExtractor"
...
public:
    ...
    bool FlacResync(const sp<DataSource> &source, uint32_t /* match_header */, off_t *inout_pos,
            uint64_t *frm_num, int maxbuffcnt);
protected:
    virtual ~FLACSource();
    ...
    switch (metadata->type)
    {
        case FLAC__METADATA_TYPE_STREAMINFO:
            if (!mStreamInfoValid)
            {
                ...
                LOGV("FLACParser::metadataCallback maxfrm %d, minfrm %d, samprate %d, tsample %llu",
                        mStreamInfo.max_framesize, mStreamInfo.min_framesize, mStreamInfo.sample_rate,
                        (unsigned long long)mStreamInfo.total_samples);
            }
            else
            {
...
status_t FLACParser::init()
{
    ...
    if (mStreamInfoValid)
    {
        ...
        if (mTrackMetadata != 0)
        {
            ...
            if (mStreamInfo.bits_per_sample > 0) mTrackMetadata->setInt32(kKeyBitWidth, mStreamInfo.bits_per_sample);
#endif
    ...
    *next_pos_offset += start_pos;
    ...
    do
    {
        ...
        if ((header & 0xffff) == kFlacMask)
        {
            ...
            if ((int64_t)(*next_pos_offset) > (buf_size - 10))
            {
                return false;
            }
            ...
            if ((int64_t)(*next_pos_offset) > (buf_size - crc_len - 1))
            {
                return false;
            }
...
Next_check:
        *next_pos_offset += 1;
        tmp += 1;
    } while ((int64_t)(*next_pos_offset) < (buf_size - 4));
...
bool FLACSource::getSeekPosforStreaming(int64_t targetTimeUS, off_t *mCurrentSeekPos, int *seekFrm)
{
    ...
    LOGD("getSeekPosforStreaming:: T_FrmNumber = %lld, Frmoffset = %d", (long long)targetFrmNum, SeekFrmOffset);
    ...
    while (1)
    {
        if (TargetFrmPos >= filesize) {
            ALOGD("TargetFrmPos(%ld), filesize(%lld)", TargetFrmPos, (long long)filesize);
            break;
        }
        ...
        if (n <= 0)
        {
            LOGD("getSeekPosforStreaming:: read file frmunm %lx is 0!!", TargetFrmPos);
            goto getSeek_Exit;
        }

        if (!FindNextSycnPos((uint8_t *)tempseekbuffer, n, 0, (uint32_t *)&next_pos_offset, &new_frmnum))
        {
            LOGD("getSeekPosforStreaming:: do not find %lx", TargetFrmPos);
            TargetFrmPos += SeekOffset;
            continue;
        }

        if (new_frmnum > (uint64_t)targetFrmNum) {
            upper_bound = TargetFrmPos;
            upper_frmnum = new_frmnum;
        } else if (new_frmnum <= (uint64_t)targetFrmNum) {
        ...
        if ((upper_bound - lower_bound) <  2 * mMetaInfo.max_framesize) break;
...
getSeek_Exit:
    ...
    if (tempseekbuffer) delete [] tempseekbuffer;
    return ret;
}
...
status_t FLACSource::start(MetaData * /* params */)
{
    ...
    ///mCurrentPos = pos;
    LOGV("FLACSource::start offset: 0x%08lx, bufsize %zu, minbuff %zu", mCurrentPos, mMaxBufferSize, mMinBufoffset);
    ...
    MediaBuffer **outBuffer, const ReadOptions *options)
{
    ...
    *outBuffer = NULL;
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode))
    {
        if (mParser->mSeekTableValid)
        {
            ...
            if (mParser->parser_getseektable((FLAC__uint64 *)&sample, (FLAC__uint64 *)&ptmpPos))
            {
                ...
                LOGD("buildin seektable seektime = %lld, actual = %lld, filePos = %ld, %d, %d",
                        (long long)seekTimeUs, (long long)seektmpTimeUs, mCurrentPos,
                ...
                {
                    int retryTimes = 10;   //Finding sync header fail, only retry 10 times.
                    while (1)
                    {
            ...
            {
                LOGD("after seek error mCurrentTimeUs = %llu, ptmpPos = %llu",
                        (unsigned long long)sample, (unsigned long long)ptmpPos);
                buffer->release();
                buffer = NULL;
                return ERROR_UNSUPPORTED;
            }
        ...
        else if (mDataSource->flags() & DataSource::kIsCachingDataSource)
        {
            ...
            LOGD("FLACSource:: stream seek start %lld", (long long)seekTimeUs);
            ...
            if (getSeekPosforStreaming(seekTimeUs, &temp_mCurrentPos, &Framenum))
            {
                ...
                LOGD("FLACSource:: stream seek done mCurrentTimeUs = %lx, ptmpPos = %lld",
                        mCurrentPos, (long long)mCurrentTimeUs);
            }
            else
            {
        ...
        else
        {
            ...
            {
                LOGE("FLACSource::seek to %lld fail, err %d !!!!!", (long long)seekTimeUs, stat);
                buffer->release();
                buffer = NULL;
                return stat;
            }
            else
            {
                ...
                LOGD("TOC seektable seektime = %lld, actual = %lld, filePos = %ld",
                        (long long)seekTimeUs, (long long)mCurrentTimeUs, mCurrentPos);
            }
        }
    }

    if (mFirstFramePos > 0 && mFirstFramePos > mCurrentPos) {
        // if mCurrentPos < mFirstFramePos, we should set mCurrentPos = mFirstFramePos,
        // otherwise it will get a wrong frame whose next frame offset is mFirstFramePos
        ALOGI("mFirstFramePos(%lld) > mCurrentPos(%lld)", (long long)mFirstFramePos, (long long)mCurrentPos);
    ...
    while (1) {
        ...
        if (n <= 0)
        {
            buffer->release();
            buffer = NULL;
            LOGD("FLACSource::readAt fail %zd %zu", n, frame_size);
            return ERROR_END_OF_STREAM;
        }
        ...
        if (FindNextSycnPos((uint8_t *)buffer->data(), frame_size, start_offset, (uint32_t *)&nextfrm_offset, NULL))
        {
            ...
            if ((long long)mCurrentPos >= (long long)(filesize - (long long)mMaxBufferSize)) {
                frame_size= n;
                break;
            }
            frame_size = mMaxBufferSize;
    ...
    *outBuffer = buffer;
    mCurrentFrm++;
    LOGV("FLACSource::kKeyTime Cur 0x%08lx, frm %ld--%lld, frm_size %zu",
            mCurrentPos, mCurrentFrm, (long long)mCurrentTimeUs, frame_size);
    return OK;
}
...
status_t FLACSource::init()
{
    ...
    if (n == 14)
    {
        ...
        unsigned blocksize = 0, sample_rate = 0, bps;
        uint32_t blk_s_len = 0;
        uint32_t rate_s_len = 0;
        uint32_t blk_bit_len = 0;
        uint16_t blk_bit = 0;
