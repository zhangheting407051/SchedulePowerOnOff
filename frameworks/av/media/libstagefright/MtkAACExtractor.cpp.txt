...
static const uint32_t AACSampleFreqTable[16] =
{
    ...
    11025, /* 11025 Hz */
    8000, /*  8000 Hz */
    (uint32_t)(-1), /* future use */
    (uint32_t)(-1), /* future use */
    (uint32_t)(-1), /* future use */
    (uint32_t)(-1)  /* escape value */
};

// Returns the frame length in bytes as described in an ADTS header starting at the given offset,
//     or 0 if the size can't be read due to a read failure.
// *pNextPos return the position of next ADTS header
static size_t getAdtsFrameLength(const sp<DataSource> &source, off_t curPos, off_t *pNextPos, uint32_t fixedHeader) {
    uint8_t tmp[7];
    size_t frame_size;
    for (;;) {
        ssize_t n = source->readAt(curPos, tmp, 7);
        if (n < 7) return 0;

        uint32_t header = U32_AT(tmp);

        if ((fixedHeader & kMask) == (header & kMask)) {
    ...
    uint32_t *pBitRate, uint32_t *pProfile, uint32_t *pChannelNum, int32_t *pIsADIF)
{
    ...
    {
        *pIsADIF = true;
        AAC_EXTR_DBG("getAACInfo -- ADIF: pos = %ld", *inout_pos);
        return true;
    }
...
bool AACFrameLenDecoder::initAACDec()
{
    ...
    uint32_t type;
    const void *data;
    size_t size[FILEREAD_MAX_LAYERS] = {0};

    if (mMeta->findData(kKeyESDS, &type, &data, &size[0])) {
        const void* codec_specific_data[FILEREAD_MAX_LAYERS];
        size_t codec_specific_data_size;
        ESDS esds((const char *)data, size[0]);
        ...
        esds.getCodecSpecificInfo(
            &(codec_specific_data[0]), &codec_specific_data_size);
        AAC_EXTR_DBG("FRAUNHOFER_AAC--initAACDec codec_specific_data_size = %zu", codec_specific_data_size);
...
status_t AACFrameLenDecoder::getNextFramePos(off_t *pCurPos, off_t *pNextPos)
{
    ...
    ALOGV("+getNextFramePos: *pCurPos = %lld, mFramePosition = %lld, mTmpBufLen = %u",
            (long long)(*pCurPos), (long long)mFramePosition, mTmpBufLen);
    ...
    if (*pCurPos != mFramePosition) {
        ...
        mTmpBufLen += size;
        mLastNumTotalBytes = 0;
        ALOGV("Seek *pCurPos = %lld, mCurFliePos = %lld, mFramePosition = %lld, mTmpBufLen = %u",
              (long long)*pCurPos, (long long)mCurFliePos, (long long)mFramePosition, mTmpBufLen);
    }
    ...
    if (mReadFlag)
    {
        if (mTmpBufLen < ADIF_DEC_INBUF_LEN / 2) //Fill
        {
            ...
            AAC_EXTR_VEB("mCurFliePos %ld, mTmpBuf %p mCurPosition %d read %d",
                    mCurFliePos, mTmpBuf, mCurPosition, ADIF_DEC_INBUF_LEN - mTmpBufLen);
        ...
        AAC_EXTR_VEB("mCurFliePos %ld mReadFlag: mTmpBufLen %d, mCurPosition %d size %zd",
                mCurFliePos, mTmpBufLen, mCurPosition, size);
    }

    AAC_DECODER_ERROR decoderErr;
    // Fill and decode
    INT_PCM outBuffer[2048 * MAX_CHANNEL_COUNT];
    memset(outBuffer, 0x00, 2048 * MAX_CHANNEL_COUNT * sizeof(INT_PCM));
    ...
    if (mTmpBufLen > 0) {
        ...
        ALOGV("mTmpBufLen = %u, bytesValid[0] = %u", mTmpBufLen, bytesValid[0]);
        int inBufferUsedLength = mTmpBufLen - bytesValid[0];

        if (AAC_DEC_OK == decoderErr && inBufferUsedLength >= 0)
        {
            *pCurPos = mFramePosition;
            if (mStreamInfo->numTotalBytes <= mLastNumTotalBytes) {
                MM_LOGW("return ERROR_END_OF_STREAM when mStreamInfo->numTotalBytes(%u) <= mLastNumTotalBytes(%u)",
                        mStreamInfo->numTotalBytes, mLastNumTotalBytes);
                return ERROR_END_OF_STREAM;
            }
            consumBsLen = mStreamInfo->numTotalBytes - mLastNumTotalBytes;
        } else {
            MM_LOGE("Error 0x%x, mStreamInfo->numTotalBytes = %u, mLastNumTotalBytes = %u",
                    decoderErr, mStreamInfo->numTotalBytes, mLastNumTotalBytes);
            *pCurPos = mFramePosition;
            ...
            ALOGV("mInterDecBufSize = %u, DecFreeBytes = %u, mStreamInfo->numTotalBytes = %u, mLastNumTotalBytes = %u",
                    mInterDecBufSize, DecFreeBytes, mStreamInfo->numTotalBytes, mLastNumTotalBytes);
            consumBsLen = (mInterDecBufSize - DecFreeBytes) + (mStreamInfo->numTotalBytes - mLastNumTotalBytes);
            //discard buffer
            aacDecoder_SetParam(mAACDecoder, AAC_TPDEC_CLEAR_BUFFER, 1);
        }

        MM_LOGV("mFramePosition = %lld, consumBsLen = %d", (long long)mFramePosition, consumBsLen);
        *pNextPos = mFramePosition + consumBsLen;
        mFramePosition += consumBsLen;
        MM_LOGV("mTmpBufLen = %u, inBufferUsedLength = %d", mTmpBufLen, inBufferUsedLength);
        ...
        else
        {
            *pCurPos = mFramePosition;
            ...
            consumBsLen = mStreamInfo->numTotalBytes - mLastNumTotalBytes;
            MM_LOGV("mFramePosition = %lld, consumeBsLen = %d", (long long)mFramePosition, consumBsLen);
            *pNextPos = mFramePosition + consumBsLen;
            mFramePosition += consumBsLen;
            mLastNumTotalBytes = mStreamInfo->numTotalBytes;
    ...
    AAC_EXTR_VEB("getNextFramePos end *pCurPos %ld,*pNextPos %ld consumBsLen %d", *pCurPos, *pNextPos, consumBsLen);
    return OK;
}
...
private:
    ...
    int64_t mDurationUsByTOC;  // duration which is calculated by TOC
    ...
    if (mDataSource->getSize(&fileSize) == OK) {
        if (bIsADIF) {
            if (nBitRate != 0) {
                ...
                ALOGI("ADIF: fileSize = %lld, nDurationUs = %llu",
                      (long long)fileSize, (unsigned long long)nDurationUs);
            }
        } else {
            off64_t numFrames = 0;
            off_t nextPos = pos;
            while (pos < fileSize) {
                if (getAdtsFrameLength(source, pos, &nextPos, mFixedHeader) == 0) {
                    break;
                }
                pos = nextPos;
                numFrames++;
            ...
            nDurationUs = numFrames * mFrameDurationUs;
            mMeta->setInt64(kKeyDuration, nDurationUs);
            ALOGI("ADTS: duration = %llu us", (unsigned long long)nDurationUs);

            // estimate bitRate
            nBitRate = 8000000LL * ((float)(fileSize - mFirstFramePos) / nDurationUs);
        }
    }

    ALOGI("MtkAACExtractor(): nBitRate = %u", nBitRate);
    ALOGI("MtkAACExtractor(): nSampleRate = %u", nSampleRate);
    ALOGI("MtkAACExtractor(): nProfile = %u", nProfile);
    ALOGI("MtkAACExtractor(): nChannelNum = %u", nChannelNum);
    ALOGI("MtkAACExtractor(): nSampleFreqIndex = %u", nSampleFreqIndex);
...
sp<IMediaSource> MtkAACExtractor::getTrack(size_t index)
{
    ...
    unsigned channel_configuration)
{
    ...
    AAC_EXTR_DBG("AACExtractor::MakeAACCodecSpecificData--csd->data()[%lu]=%x",
            (unsigned long)sizeof(kStaticESDS), csd->data()[sizeof(kStaticESDS)]);
    AAC_EXTR_DBG("AACExtractor::MakeAACCodecSpecificData--csd->data()[%lu]=%x",
            (unsigned long)(sizeof(kStaticESDS) + 1), csd->data()[sizeof(kStaticESDS) + 1]);
...
status_t MtkAACSource::start(MetaData *)
{
    ...
    if (mMeta->findInt32(kKeySampleRate, &nSampleRate)) {
        mFrameDurationUs = (1024 * 1000000ll + (nSampleRate - 1)) / nSampleRate;
        AAC_EXTR_DBG("Per Frame Duration = %lld, First frame pos = %ld", (long long)mFrameDurationUs, mFirstFramePos);
    ...
    *out = NULL;
    ...
    if (mIsADTS)
    {
        ...
        if (options != NULL && options->getSeekTo(&seekTimeUs, &mode))
        {
            ...
            AAC_EXTR_DBG("MtkAACSource::read--seek mCurrentPos = %ld", mCurrentPos);
        }
