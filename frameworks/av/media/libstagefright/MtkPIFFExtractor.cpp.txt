...
//#define LOG_NDEBUG 0
#define LOG_TAG "MtkPIFFExtractor"
...
#include "include/MtkPIFFExtractor.h"
...
class MtkPIFFSource : public MediaSource {
public:
    // Caller retains ownership of both "dataSource" and "sampleTable".
    MtkPIFFSource(const sp<MtkPIFFExtractor> &owner,
                const sp<MetaData> &format,
                const sp<DataSource> &dataSource,
                int32_t timeScale,
                const sp<SampleTable> &sampleTable,
                Vector<MtkPiffSidxEntry> &sidx,
#ifdef MTK_PLAYREADY_SUPPORT
                Vector<MfraEntry> &mfra,
#endif
                const MtkPiffTrex *trex,
...
protected:
    virtual ~MtkPIFFSource();

private:
    Mutex mLock;

    // keep the MtkPIFFExtractor around, since we're referencing its data
    sp<MtkPIFFExtractor> mOwner;
    sp<MetaData> mFormat;
    ...
    Vector<MtkPiffSidxEntry> &mSegments;
#ifdef MTK_PLAYREADY_SUPPORT
    // for build initialized
    Vector<MfraEntry> &mMovieFragments;
#endif
    const MtkPiffTrex *mTrex;
    ...
    MtkPIFFSource(const MtkPIFFSource &);
    MtkPIFFSource &operator=(const MtkPIFFSource &);
public:
...
// Each MtkPIFFDataSource caches the sampletable metadata for a single track.

struct MtkPIFFDataSource : public DataSource {
    MtkPIFFDataSource(const sp<DataSource> &source);
...
private:
    ...
    MtkPIFFDataSource(const MtkPIFFDataSource &);
    MtkPIFFDataSource &operator=(const MtkPIFFDataSource &);
};

MtkPIFFDataSource::MtkPIFFDataSource(const sp<DataSource> &source)
    : mSource(source),
      mCachedOffset(0),
      mCachedSize(0),
      mCache(NULL) {
...
MtkPIFFDataSource::~MtkPIFFDataSource() {
    clearCache();
}

void MtkPIFFDataSource::clearCache() {
    if (mCache) {
        free(mCache);
        mCache = NULL;
    }
...
status_t MtkPIFFDataSource::initCheck() const {
    return mSource->initCheck();
}

ssize_t MtkPIFFDataSource::readAt(off64_t offset, void *data, size_t size) {
...
status_t MtkPIFFDataSource::getSize(off64_t *size) {
    return mSource->getSize(size);
}

uint32_t MtkPIFFDataSource::flags() {
    return mSource->flags();
}

status_t MtkPIFFDataSource::setCachedRange(off64_t offset, size_t size) {
    Mutex::Autolock autoLock(mLock);
...
MtkPIFFExtractor::MtkPIFFExtractor(const sp<DataSource> &source)
    : mMoofOffset(0),
      mMoofFound(false),
      mMdatFound(false),
      mDataSource(source),
...
MtkPIFFExtractor::~MtkPIFFExtractor() {
...
uint32_t MtkPIFFExtractor::flags() const {
#ifdef MTK_PLAYREADY_SUPPORT
    return CAN_PAUSE |
...
sp<MetaData> MtkPIFFExtractor::getMetaData() {
    status_t err;
...
size_t MtkPIFFExtractor::countTracks() {
    status_t err;
    if ((err = readMetaData()) != OK) {
        ALOGV("MtkPIFFExtractor::countTracks: no tracks");
        return 0;
    }
    ...
    ALOGV("MtkPIFFExtractor::countTracks: %zu tracks", n);
    return n;
}

sp<MetaData> MtkPIFFExtractor::getTrackMetaData(
...
status_t MtkPIFFExtractor::readMetaData() {
    ...
    if(ret != OK) {
        // ALPS02526427 do not return error, due to cts case test3gppMetadata
        // the case use dataSource,it do not support getSize
        ALOGW("Warning:getSize ret:%d", ret);
    } else {
        uint32_t hdr[2];
...
char* MtkPIFFExtractor::getDrmTrackInfo(size_t trackID, int *len) {
    if (mFirstSINF == NULL) {
        return NULL;
    }
...
#endif
status_t MtkPIFFExtractor::parseDrmSINF(
...
status_t MtkPIFFExtractor::parseChunk(off64_t *offset, int depth) {
        ...
        case FOURCC('e', 'd', 't', 's'):
        {
            ...
            if (chunk_type == FOURCC('s', 't', 'b', 'l')) {
                ...
                if (mDataSource->flags()
                    ...
                    sp<MtkPIFFDataSource> cachedSource =
                        new MtkPIFFDataSource(mDataSource);
...
status_t MtkPIFFExtractor::parseSegmentIndex(off64_t offset, size_t size) {
  ALOGV("MtkPIFFExtractor::parseSegmentIndex");
...
status_t MtkPIFFExtractor::parseQTMetaKey(off64_t offset, size_t size) {
    if (size < 8) {
        return ERROR_MALFORMED;
    }
...
status_t MtkPIFFExtractor::parseQTMetaVal(
...
status_t MtkPIFFExtractor::parseTrackHeader(
...
status_t MtkPIFFExtractor::parseITunesMetaData(off64_t offset, size_t size) {
...
status_t MtkPIFFExtractor::parse3GPPMetaData(off64_t offset, size_t size, int depth) {
    if (size < 4 || size == SIZE_MAX) {
        return ERROR_MALFORMED;
    }
...
void MtkPIFFExtractor::parseID3v2MetaData(off64_t offset) {
...
sp<IMediaSource> MtkPIFFExtractor::getTrack(size_t index) {
    status_t err;
    if ((err = readMetaData()) != OK) {
        return NULL;
    }
    ...
    MtkPiffTrex *trex = NULL;
    int32_t trackId;
    if (track->meta->findInt32(kKeyTrackID, &trackId)) {
        for (size_t i = 0; i < mTrex.size(); i++) {
            MtkPiffTrex *t = &mTrex.editItemAt(index);
...
// static
status_t MtkPIFFExtractor::verifyTrack(Track *track) {
    const char *mime;
    CHECK(track->meta->findCString(kKeyMIMEType, &mime));
...
status_t MtkPIFFExtractor::updateAudioTrackInfoFromESDS_MPEG4Audio(
...
MtkPIFFSource::MtkPIFFSource(
        const sp<MtkPIFFExtractor> &owner,
        const sp<MetaData> &format,
        const sp<DataSource> &dataSource,
        int32_t timeScale,
        const sp<SampleTable> &sampleTable,
...
status_t MtkPIFFSource::start(MetaData *params) {
    Mutex::Autolock autoLock(mLock);
...
#ifdef MTK_PLAYREADY_SUPPORT
status_t MtkPIFFSource::parseChunk(off64_t *offset, MfraEntry *mfra) {
#else
status_t MtkPIFFSource::parseChunk(off64_t *offset) {
#endif
    uint32_t hdr[2];
    if (mDataSource->readAt(*offset, hdr, 8) < 8) {
        return ERROR_IO;
    ...
    char chunk[5];
    MakeFourCCString(chunk_type, chunk);
    ALOGV("MtkPIFFSource chunk %s @ %#llx", chunk, (long long)*offset);
    ...
    switch(chunk_type) {
        ...
        *offset += chunk_size;
        ...
        case FOURCC('m', 'd', 'a', 't'): {
            ...
            ALOGV("MtkPIFFSource::parseChunk mdat");
...
status_t MtkPIFFSource::parseSampleAuxiliaryInformationSizes(
...
status_t MtkPIFFSource::parseSampleAuxiliaryInformationOffsets(
        off64_t offset, off64_t /* size */) {
...
status_t MtkPIFFSource::parseTrackFragmentHeader(off64_t offset, off64_t size) {
...
#ifdef MTK_PLAYREADY_SUPPORT
status_t MtkPIFFSource::parseTrackFragmentRun(off64_t offset, off64_t size, uint32_t sampleNum) {
#else
status_t MtkPIFFSource::parseTrackFragmentRun(off64_t offset, off64_t size) {
#endif

    ALOGV("MtkPIFFExtractor::parseTrackFragmentRun");
    if (size < 8) {
        return -EINVAL;
    }
...
sp<MetaData> MtkPIFFSource::getFormat() {
...
size_t MtkPIFFSource::parseNALSize(const uint8_t *data) const {
...
status_t MtkPIFFSource::fragmentedRead(
        MediaBuffer **out, const ReadOptions *options) {

    ALOGV("MtkPIFFSource::fragmentedRead");
    ...
    if (options && options->getSeekTo(&seekTimeUs, &mode)) {
        ...
        if (numSidxEntries != 0) {
            ...
            for (int i = 0; i < numSidxEntries; i++) {
                const MtkPiffSidxEntry *se = &mSegments[i];
...
MtkPIFFExtractor::Track *MtkPIFFExtractor::findTrackByMimePrefix(
...
static bool BetterSniffPIFF(
    ...
    ALOGW("foundGoodFileType:%d", foundGoodFileType);
    if (foundGoodFileType) {
        if (LegacySniffPIFF(source)) {
            *mimeType = "video/ismv";
            *confidence = 0.5f;
            ALOGW("LegacySniffPIFF OK, Is Piff");
            return true;
        }
    }
...
bool SniffMtkPIFF(
        const sp<DataSource> &source, String8 *mimeType, float *confidence,
        sp<AMessage> *) {
    if (BetterSniffPIFF(source, mimeType, confidence)) {
        return true;
    }
...
status_t MtkPIFFExtractor::setCodecInfoFromFirstFrame(Track *track)
{
...
status_t MtkPIFFExtractor::setMetaData() {
...
void MtkPIFFSource::clearEncryptedKeyMetaData(const sp<MetaData> &metadata) {
    int64_t timeUs = -1;
...
bool MtkPIFFSource::setDataSource(const sp<DataSource> &dataSource)
{
    Mutex::Autolock autoLock(mLock);
    mDataSource = dataSource;
    return true;
...
status_t MtkPIFFExtractor::parseDrmSINFMOOV(off64_t* /*offset*/, off64_t data_offset) {
...
status_t MtkPIFFExtractor::parseDrmSINFTrack(off64_t* /*offset*/, off64_t data_offset) {
    uint8_t buf[40];
    if (mDataSource->readAt(
        data_offset, buf, sizeof(buf))
        < (ssize_t)sizeof(buf)) {
...
*/
status_t MtkPIFFExtractor::parseMovieFragment(off64_t offset, size_t size) {
    ALOGD("MtkPIFFExtractor::parseMovieFragment");
   ...
   // get track meta and timeScale
    ...
    while (true) {
        if (track == NULL) {
            ...
            int32_t trackidtmp;
            CHECK(track->meta->findInt32(kKeyTrackID, &trackidtmp));
            if (trackid == (uint32_t)trackidtmp) {
                break;
            }
        }
        track = track->next;
    }
    uint32_t timescale = (track->timescale != 0) ? track->timescale : mHeaderTimescale;
    ...
    for(uint32_t i = 0; i < number_of_entry; i++) {
        ...
        if(version == 1){
            ...
            ALOGI("mfra add, mTime:%lld", (long long)me.mTime);
        ...
        mfraEntrys.add(me);
        total_duration = me.mTime;
        delete [] data;
    }

    mMfraEntries.add(trackid, mfraEntrys);
    ALOGI("mMfraEntries add");

    // for ffmpeg ismv file, track duration is 0, so use mfra to evaluate the duration
    int64_t metaDuration;
    if (!track->meta->findInt64(kKeyDuration, &metaDuration) || metaDuration == 0) {
        if (timescale != 0) {
            uint64_t mfraDuration = total_duration * 1000000 / timescale;
            track->meta->setInt64(kKeyDuration, mfraDuration);
            ALOGI("track duraion:%lld, timescale:%u", (long long)mfraDuration, timescale);
        }
    }
    return OK;
}

status_t MtkPIFFSource::parseAVCCodecSpecificData(
        const void *data, size_t size,
        unsigned *profile, unsigned *level) {
    const uint8_t *ptr = (const uint8_t *)data;
...
void MtkPIFFSource::addCodecSpecificData(const void *data, size_t size) {
...
void MtkPIFFSource::clearCodecSpecificData() {
...
status_t MtkPIFFSource::parseSampleUUid(off64_t offset, off64_t /*size*/) {
    uint8_t header[20];
    if (mDataSource->readAt(
...
#endif
    int ivlength;
    if (!mFormat->findInt32(kKeyCryptoDefaultIVSize, &ivlength)) {
        ALOGW("Warning: uuid no kKeyCryptoDefaultIVSize");
        return OK;
    }
    CHECK_EQ(ivlength, 8);
