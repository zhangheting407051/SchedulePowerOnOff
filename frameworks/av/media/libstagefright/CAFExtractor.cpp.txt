      ...
      mDataStartPos(0)
{
    ...
    ALOGD("  mNumberPackets:     %lld",       (long long)(mCAFPaktChunkInfo.nNumberPackets));
    ALOGD("  mNumberValidFrames: %lld",       (long long)(mCAFPaktChunkInfo.nNumberValidFrames));
    ...
    ALOGD("  mPaktStartPos: %lld", (long long)mPaktStartPos);
    ALOGD("  mDataStartPos: %lld", (long long)mDataStartPos);
...
status_t CAFSource::getNextFramePos(off_t *pCurpos, off_t *pNextPos, int64_t *frameTsUs)
{
    if (mBytesPerPacket != 0 && mFramesPerPacket == 0)
    {
        ...
        ALOGV("CurPos = %ld, CurPaktPos = %ld", *pCurpos, mCurPaktPos);
        ...
        *pNextPos = *pCurpos + mBytesPerPacket;
        ...
        ALOGV("frameSize = %u, NextPos = %ld, frameUs = %lld, CurPaktPos = %ld, NextPaktPos = %lld",
            mBytesPerPacket, *pNextPos, (long long)(*frameTsUs), mCurPaktPos, (long long)mNextPaktPos);
    }
    else if (mBytesPerPacket == 0 && mFramesPerPacket != 0)
    {
        ...
        *pNextPos = *pCurpos + frameSize;
        mNextPaktPos = paktPos;
        ALOGV("frameSize = %zu, NextPos = %ld, frameUs = %lld, CurPaktPos = %ld, NextPaktPos = %ld",
            frameSize, *pNextPos, (long long)(*frameTsUs), mCurPaktPos, mNextPaktPos);
    }
    else if (mBytesPerPacket == 0 && mFramesPerPacket == 0)
    {
        uint8_t data = 0;
   ...
   // int64_t seekFrame = 0;
    ...
    if (options != NULL && options->getSeekTo(&seekTimeUs, &mode)) {
        ...
        if ((mDurationusBytoc != -1) && (seekTimeUs > mDurationusBytoc))
        {
            ...
            ALOGD("seekTimeUs change to duration when seekTimeUs > mDuration");
        }
        ...
        ALOGD("seek = true, mCurrentTimeUs = %lld, mOffset = %ld, mPaktOffset = %ld", (long long)mCurrentTimeUs, mOffset, mPaktOffset);
