...
bool DataSource::fastsniff(
    ...
    *mimeType ="";
    ...
    memset(linkto, 0, SIZE);
    snprintf(buffer, SIZE, "/proc/%d/fd/%d", gettid(), fd);
    int len = 0;
    ...
    ALOGV("fastsniff pid %d, fd %d, len=%d", gettid(), fd, len);
...
#endif
#if defined(MTK_PLAYREADY_SUPPORT) && defined(MTK_PLAYREADY_FAKEMODE)
    RegisterSniffer_l(SniffPRFake);
    RegisterSniffer_l(SniffMtkPIFF);
#endif
}
