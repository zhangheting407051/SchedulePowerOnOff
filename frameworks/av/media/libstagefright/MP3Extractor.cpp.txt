...
sp<MetaData> MP3Extractor::getTrackMetaData(
    ...
    {
        mMeta->setInt32(kKeyIsMtkMusic, 1);
        ALOGD("Get Low power flag");
    }
...
MP3Source::MP3Source(
    ...
    mDurationUsByTOC = -1;//notify toc duration async
...
status_t MP3Source::MTK_read(
    ...
    *out = NULL;
    ...
    if(0 != sample_rate) {
        ...
        ALOGE("Got sample_rate = %d, Signalling end of stream.",sample_rate);
        buffer->release();
        buffer = NULL;
        return ERROR_END_OF_STREAM;
    }
    ...
    if(mDataSource->flags() & DataSource::kIsCachingDataSource){//streaming using
        ...
        if (mSeeker == NULL || !mSeeker->getDuration(&durationUsStream)){
            ...
            if (mDataSource->getSize(&fileSize_Stream) == OK) {
                if (fileSize_Stream > mFirstFramePos)
                {
        ...
        if (!specialheader && mDataSource->getSize(&fileSize_Enh) == OK){
            ...
            {
                if (fileSize_Enh > mFirstFramePos)
                {
