...
DirectRenderer::AudioRenderer::AudioRenderer(
    ...
    ALOGE("AudioRenderer::AudioRenderer audiotrack framecount = %zu", tempFrameCnt);
...
//#ifdef MTK_AOSP_ENHANCEMENT
    ...
    ALOGD("AccessUnitDelayUs = %lld us",(long long)AccessUnitDelayUs);
    ...
    ALOGD("numFramesPlayed = %d ,PengdinginAudiotrack %d frames delayUs %lld us,PendinginOutPutbuffer %d frames delayUs = %lld us,TotaldelayUs = %lld us,",
    ...
    if(AccessUnitDelayUs > 500000){
        ALOGD("audio Accessunit pending too much data,drop some frame for audio latency");
        mDecoderContext->releaseAccessunit();
    }else if (TotaldelayUs > 180000 && delayUs > 80000){
        ALOGD("drop some outputbuffer for latency");
...
void DirectRenderer::onRenderVideo() {
    ...
    //mediatype: true:video,false:audio;
        ...
        ALOGD("t1 %lld, ts %lld", (long long)t1, (long long)timestamp);
