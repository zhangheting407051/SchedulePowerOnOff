    ...
    :mMetaData(new MetaData), mSessionDesc(new ASessionDescription)
{
    ...
    if (source->getSize(&fileSize) != OK) {
        fileSize = 4096 * 2;
        ALOGW("no lenth of SDP, try max of %lld", (long long)fileSize);
    }
    ...
    if (data != NULL) {
        ...
        if (n > 0) {
            if (n != fileSize) {
                ALOGW("data read may be incomplete %d vs %lld", (int)n, (long long)fileSize);
            }
            mSessionDesc->setTo(data, n);
            ((char *)data)[n] = '\0';
            mMetaData->setCString(kKeySDP, (char *)data);
            mMetaData->setInt32(kKeySDPLen, n);
        }
        free(data);
    } else {
...
sp<IMediaSource> MtkSDPExtractor::getTrack(size_t index) {
    if(index == 0) ALOGI("WR");
    return NULL;
}
