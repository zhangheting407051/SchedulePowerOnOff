...
sp<IMediaSource> ESExtractor::getTrack(size_t /*index*/) {
    if (mTrack == NULL) {
        return NULL;
    }
...
status_t ESExtractor::feedMore() {
    ...
    for (;;) {
        ...
        if (err == -EAGAIN && mFinalResult == OK) {
            ...
            ALOGD("mBuffer->size():%d,kChunkSize:%d,mBuffer->capacity():%d",(int)mBuffer->size(),(int)kChunkSize,(int)mBuffer->capacity());
            if (mBuffer->size() + kChunkSize > mBuffer->capacity()) {
                ...
                ALOGD("Capacity %d->%d\n", (int)mBuffer->capacity(), (int)newCapacity);
            ...
            mOffset += n;
            ALOGD("Read success,mBuffer->size()%d",(int)mBuffer->size());
        }
        else if (err != OK) {
            mFinalResult = err;
            return err;
...
status_t ESExtractor::Track::read(
    ...
    while (!mSource->hasBufferAvailable(&finalResult)) {
        ALOGD("mSource has no Buffer Available,finalResult:%d",(int)finalResult);
        if (finalResult != OK) {
    ...
    while (offset + 3 < size) {
        ...
        if ((prevStartCode == 0xb3 && currentStartCode != 0xb5)
            ...
            if (mQueueFormat == NULL) {
                ...
                ALOGI("found MPEG2 video codec config (%d x %d)", (int)width, (int)height);
...
//  ALOGV("dequeueAccessUnitHEVC Start,size:%d",size);
    ...
    ssize_t preVCLIndex = -1;
    ssize_t preSliceType = 0, curSliceType = 0;

    while ((err = getNextNALUnit4(&data, &size, &nalStart, &nalSize)) == OK) {

        CHECK_GT(nalSize, 0u);

        ssize_t sliceType = (nalStart[0] & 0x7E)>>1;
        ...
        if (sliceType == 35) {
            ...
            ssize_t firstSlice = (nalStart[2] & 0x80)>>7;
        ...
        if (flush) {
            ...
            size_t dstOffset = 0;
            for (ssize_t i = 0; i < preVCLIndex; ++i) {
                const NALPosition2 &pos = nals.itemAt(i);
            ...
            const NALPosition2 &pos = nals.itemAt(preVCLIndex - 1);
        ...
        if (32 == curSliceType) {
            ...
            const NALPosition2 &pos = nals.itemAt(nals.size() - 1);
            ...
            if (mQueueFormat == NULL) {
                //mQueueFormat = MakeHEVCMetaData(mAccessUnit);
                mQueueFormat = MakeHEVCCodecSpecificData(mAccessUnit);
            }
