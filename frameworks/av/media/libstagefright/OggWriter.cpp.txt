...
status_t OggWriter::addSource(const sp<IMediaSource> &source)
{
...
status_t OggWriter::threadFunc()
{
    ...
    while (!mDone) {
        ...
        LOGV("OggWriter::threadFunc:buffer->range_length()=%zu", buffer->range_length());
        ...
        if (mPaused || buffer->range_length() == 0)
        {
            ...
            if (mPaused && mPausedflag)
            {
                ...
                LOGD("OggWriter::threadFunc,pausetime=%d,tsUsPauseBeg=%lld", iPausedTime, (long long)tsUsPauseBeg);
                mPausedflag =  false;
            }
            ...
            if (buffer->range_length() > 0)
            {
                ...
                if (memcmp((char *)buffer->data() + 29, "vorbis", 6) != 0)
                {
                    buffer->release();
                    buffer = NULL;
                    continue;
                ...
                {
                    LOGD("ogg header:buffer=%p,size=%zu", buffer, buffer->range_length());
                }
            }
            else
            {
        ...
        if (timestampUs > 0)
        {
            ...
            if (mResumed)
            {
                ...
                LOGD("previousPausedDurationUs =%lld,pausetime=%d,tsUsPauseBeg=%lld,tsUsPauseEnd=%lld",
                     (long long)previousPausedDurationUs, iPausedTime, (long long)tsUsPauseBeg, (long long)tsUsPauseEnd);
                ...
                LOGD("previousPausedDurationUs =%lld,samplecount=%lld",
                    (long long)previousPausedDurationUs, (long long)smpPaused);
                mResumed = false;
            }

            timestampUs -= previousPausedDurationUs;
            LOGV("time stamp: %lld, previous paused duration: %lld",
                (long long)timestampUs, (long long)previousPausedDurationUs);
