...
status_t AudioTrackCenter::init() {
    ...
    if (!mAfFrameCount || !mAfSampleRate) {
        ...
        SLOGD("init, mAfFrameCount = %zu, mAfSampleRate = %d",mAfFrameCount, mAfSampleRate);
    }
...
status_t AudioTrackCenter::addTrack(intptr_t trackId, uint32_t frameCount, uint32_t sampleRate, void* trackPtr,  uint32_t afFrameCount, uint32_t afSampleRate, uint32_t framesFilled) {
    ...
    ALOGD("addTrack: trackPtr = %p, trackId = %p, mAfSampleRate = %d, sampleRate = %d, AfFrameCount = %zu , mAfSampleRate = %d, frameCount = %d, framesFilled = %d",
            trackPtr, (void*)trackId, mAfSampleRate, sampleRate, mAfFrameCount, mAfSampleRate, frameCount, framesFilled);
    ...
    ALOGD("addTrack: trackId, info.server, info.framePlayed, info.ts, %p, %d, %" PRId64 ", %" PRId64,
...
status_t AudioTrackCenter::removeTrack(void* trackPtr) {
    ...
    ALOGD("%s, trackPtr: %p", __FUNCTION__, trackPtr);
...
status_t AudioTrackCenter::updateServer(intptr_t trackId, uint32_t server, bool restore) {
    ...
    info.server = server;
#if defined(CONFIG_MT_ENG_BUILD)
    ALOGV("updateServer: trackId, info.server, info.framePlayed, info.ts, %p, %d, %" PRId64 ", %" PRId64,
            (void*)trackId, info.server, info.framePlayed, info.ts);
#endif
    info.middleServer  = server;
...
status_t AudioTrackCenter::getRealTimePosition(intptr_t trackId, int64_t *position) {
    ...
    *position = info.framePlayed;
    ...
    if (!info.framePlayed) {
        SLOGV("trackId = %p, server = %d, framePlayed = %" PRId64, (void*)trackId, info.server, info.framePlayed);
        return OK;
    }
...
#endif
#if defined(CONFIG_MT_ENG_BUILD)
    ALOGD("getRealTimePosition: trackId, server, framePlayed, delayUs, *position = %p, %d, %" PRId64 ", %" PRId64 ", %" PRId64,
