...
AudioPCMxWay::~AudioPCMxWay()
{
    ...
    if (mStatus == NO_ERROR)
    {
        ...
        //stop();
        ALOGD("Enter AudioPCMxWay::~AudioPCMxWay(),mAudioPCMxWayThread = %p", mAudioPCMxWayThread.get());
    ...
    ALOGD("Exit AudioPCMxWay::~AudioPCMxWay()");
}
...
ssize_t AudioPCMxWay::write(const void *buffer, size_t userSize)
{
    ...
    if (ssize_t(userSize) < 0)
    {
        ...
        ALOGE("AudioPCMxWay::write(buffer=%p, size=%zu (%zu)",
...
ssize_t AudioPCMxWay::read(void *buffer, size_t userSize)
{
    ...
    {
        // sanity-check. user is most-likely passing an error code.
        ALOGE("AudioRecord::read(buffer=%p, size=%zu (%zu)",
               buffer, userSize, userSize);
        return BAD_VALUE;
    }
...
bool AudioPCMxWay::processAudioBuffer()
{
    ...
    do
    {
        ...
        if (mCblkMemory != 0)
        {
            ...
            ALOGD("%s %p", __FUNCTION__, audioBuffer.raw  );
        }
        else{
        ALOGE("no memory allocated");
        return false;
        }
