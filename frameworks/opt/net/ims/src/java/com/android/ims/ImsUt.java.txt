...
public class ImsUt implements ImsUtInterface {
    ...
    /// M: @{
    private IImsUtListenerProxy mListener = null;
    /// @}
    public ImsUt(IImsUt iUt) {
        miUt = iUt;
        /// M: @{
        mListener = new IImsUtListenerProxy();
        /// @}
        if (miUt != null) {
            try {
                /// M: @{
                miUt.setListener(mListener);
                /// @}
    ...
    public void updateListener() {
        log("updateListener: miUt=" + miUt + ", mListener=" + mListener);
        if (miUt == null || mListener == null) {
            return;
        }
        try {
            miUt.setListener(mListener);
