...
public class WifiQualifiedNetworkSelector {
    ...
    public boolean userSelectNetwork(int netId, boolean persist) {
        ...
        ///M: for operator plugin @{
        if (mWifiFwkExt != null && mWifiFwkExt.hasNetworkSelection() == IWifiFwkExt.OP_03) {
            if (mLastUserSelectSecuredNetworkForOP03 == null) {
                mLastUserSelectSecuredNetworkForOP03 = selected;
            } else if (compareSecurity(selected, mLastUserSelectSecuredNetworkForOP03) <= 0) {
                mLastUserSelectSecuredNetworkForOP03 = selected;
            }
            localLog("OP_03 mLastUserSelectSecuredNetworkForOP03:"
                    + mLastUserSelectSecuredNetworkForOP03.SSID);
        }
        ///@}
    ...
    public WifiConfiguration selectQualifiedNetwork(boolean forceSelectNetwork ,
        ...
        //Dump the scanDetails for debugging
        if (mDbg) {
            StringBuffer dumpScanResult = new StringBuffer();
            for (ScanDetail scanDetail : mScanDetails) {
                ScanResult scanResult = scanDetail.getScanResult();
                dumpScanResult.append(toScanId(scanResult) + " / ");
            }
            localLog(dumpScanResult
                + " dump for scanDetails at begin of selectQualifiedNetwork()\n");
        }
        ...
        for (ScanDetail scanDetail : mScanDetails) {
            ...
            for (WifiConfiguration network : associatedWifiConfigurations) {
                ...
                if (!status.isNetworkEnabled()) {
                    localLog("SSID[" + network.SSID + "] skips calculateBssidScore process due to"
                    + " network is not enabled(" + status.getNetworkDisableReasonString() + ")");
                    continue;
                ...
                if (mWifiFwkExt != null &&
                    ...
                    mWifiFwkExt.hasNetworkSelection() != IWifiFwkExt.OP_03) {
                    int order = 0;
                    ...
                    if (order == 0) {
                        ...
                        if (potentialCandidate.priority > network.priority) {
                            // a is of higher priority - descending
                            localLog("network select -1" + potentialCandidate.configKey() + " over "
                                        + network.configKey() + " due to priority");
                            order = -1;
                        } else if (potentialCandidate.priority < network.priority) {
                            // a is of lower priority - ascending
                            localLog("network select +1" + network.configKey() + " over "
                                        + potentialCandidate.configKey() + " due to priority");
                            order = 1;
                        }
                    }
                ...
                if (mWifiFwkExt != null &&
                    ...
                    if (mLastUserSelectSecuredNetworkForOP03 != null &&
                        mLastUserSelectSecuredNetworkForOP03.SSID.equals(networkCandidate.SSID)) {
                        localLog(networkCandidate.SSID + " is the last user select secured network"
                                + ", skip all the highestScore update process!");
                        continue;
                    }
                    int order = compareSecurity(network, networkCandidate);
                    localLog("OP_03:compareSecurity("
                            + network.SSID + "," + networkCandidate.SSID + "):" + order);
                    if (order < 0) {
                        localLog(network.SSID + " security priority higher than networkCandidate"
                                + ", reset the highestScore!");
                        highestScore = Integer.MIN_VALUE;
                        status.setCandidateScore(Integer.MIN_VALUE);
                        currentHighestScore = Integer.MIN_VALUE;
                    } else if (order > 0) {
                        localLog(network.SSID + " security priority lower than networkCandidate"
                                + ", skip the highestScore update process!");
                        continue;
                    } else if (mLastUserSelectSecuredNetworkForOP03 != null &&
                            mLastUserSelectSecuredNetworkForOP03.SSID.equals(network.SSID)) {
                        localLog(network.SSID + " is the last user select secured network"
                                + ", reset the highestScore forcely!");
                        highestScore = Integer.MIN_VALUE;
                        status.setCandidateScore(Integer.MIN_VALUE);
                        currentHighestScore = Integer.MIN_VALUE;
                    }
        ...
        if (scanResultCandidate != null) {
            localLog("HighestScoreCandate:" + networkCandidate.SSID
                      + " ConnectChoice:"
                      + networkCandidate.getNetworkSelectionStatus().getConnectChoice());
