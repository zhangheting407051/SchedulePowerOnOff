...
public class WifiScanningServiceImpl extends IWifiScanner.Stub {
    ...
    class WifiSingleScanStateMachine extends StateMachine implements WifiNative.ScanEventHandler {
        ...
        public void onFullScanResult(ScanResult fullScanResult, int bucketsScanned) {
            if (DBG) localLog("WifiSingleScanStateMachine onFullScanResult " +
                "received bucketsScanned =" +
                bucketsScanned + "fullScanResult" + fullScanResult);
        ...
        class IdleState extends State {
            @Override
            public void enter() {
                Log.e(TAG, "WifiSingleScanStateMachine IdleState enter");
                tryToStartNewScan();
            }
        ...
        void tryToStartNewScan() {
            ...
            localLog("tryToStartNewScan startSingleScan: base_period_ms" + settings.base_period_ms +
    ...
    class WifiBackgroundScanStateMachine extends StateMachine
        ...
        @Override
        public void onFullScanResult(ScanResult fullScanResult, int bucketsScanned) {
            if (DBG) localLog("WifiBackgroundScanStateMachine onFullScanResult received " +
                fullScanResult);
            sendMessage(CMD_FULL_SCAN_RESULTS, 0, bucketsScanned, fullScanResult);
        }
        ...
        private void sendBackgroundScanFailedToAllAndClear(int reason, String description) {
            ...
            mActiveBackgroundScans.clear();
            ///M: ALPS02813327: reset mPreviousSchedule due to when tur on/off wifi fast,
            /// remove scan request command may be cancelled
            mPreviousSchedule = null;
        }
                ...
                switch (msg.what) {
                    ...
                    case WifiScanner.CMD_SCAN_RESULT:
                        ...
                        if (mScanResultsPending) {
                            ...
                            ///M: google issue ClassCastException @{
                            WifiScanner.ParcelableScanData parcelableScanData =
                                (WifiScanner.ParcelableScanData) msg.obj;
                            ScanData[] scanDatas = parcelableScanData.getResults();

                            reconfigureScan(scanDatas,
    ...
    static String describeTo(StringBuilder sb, ScanSettings scanSettings) {
        ...
        sb.append(" ] ");
        sb.append(" maxPeriodInMs:").append(scanSettings.maxPeriodInMs)
          .append(" stepCount:").append(scanSettings.stepCount)
          .append(" } ");
        return sb.toString();
    }
