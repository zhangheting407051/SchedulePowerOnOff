...
public class RatConfiguration {
    ...
    static final String PROPERTY_OPT_C2K_SUPPORT="ro.boot.opt_c2k_support";
    static final String PROPERTY_OPT_LTE_SUPPORT="ro.boot.opt_lte_support";
    static final String PROPERTY_OPT_MD1_SUPPORT="ro.boot.opt_md1_support";
    ...
    private static int modemModeToRat(int modemMode) {
        int iRat = 0;
        if (modemMode == MD_MODE_LTG) {
            iRat = (MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LTTG) {
            iRat = (MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LWTG) {
            iRat = (MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_WCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LFWG) {
            iRat = (MASK_LteFdd |
                    MASK_WCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LWG ) {
            iRat = (MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_WCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LWCTG) {
            iRat = (MASK_CDMA |
                    MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_WCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LCTG) {
            iRat = (MASK_CDMA |
                    MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LTCTG) {
            iRat = (MASK_CDMA |
                    MASK_LteTdd |
                    MASK_TDSCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LFWCG) {
            iRat = (MASK_CDMA |
                    MASK_LteFdd |
                    MASK_WCDMA |
                    MASK_GSM);
        } else if (modemMode == MD_MODE_LWCG) {
            iRat = (MASK_CDMA |
                    MASK_LteFdd |
                    MASK_LteTdd |
                    MASK_WCDMA |
        ...
        logd("modemModeToRat: modem=" + modemMode+ "rat=" + iRat );
        return iRat;
    }

    private static int bootOptToRat() {
        boolean c2k_support =
                (SystemProperties.getInt(PROPERTY_OPT_C2K_SUPPORT, 1) == 1);
        boolean lte_support =
                (SystemProperties.getInt(PROPERTY_OPT_LTE_SUPPORT, 1) ==1);
        int modem_capbability =
                modemModeToRat(SystemProperties.getInt(PROPERTY_OPT_MD1_SUPPORT, 0));
        int opt_rat = RatConfiguration.MASK_WCDMA |
                RatConfiguration.MASK_TDSCDMA |
                RatConfiguration.MASK_GSM;
        if (c2k_support) {
            opt_rat |= RatConfiguration.MASK_CDMA;
        }
        if (lte_support) {
            opt_rat |= RatConfiguration.MASK_LteFdd;
            opt_rat |= RatConfiguration.MASK_LteTdd;
        }
        int iRat = modem_capbability & opt_rat;
        return iRat;
    }
    /*
    ...
    protected static int getRatConfig() {
        ...
        if (rat.length() == 0) {
            ...
            actived_rat = getMaxRat() & bootOptToRat();
            forceUpdateRatConfig(actived_rat);
        } else {
    ...
    /*
     * force to save rat to system property
     * @params int iRat, the rat to saved.
     */
    private static void forceUpdateRatConfig(int iRat) {
        logd("forceUpdateRatConfig: " + ratToString(iRat) + " "+iRat);
    }

    /*
     * save rat in bitmask to system property.
     * @params int iRat, the rat to be saved.
     * @return boolean,
     *              true, set successfully.
     *              false, the rat is not supported.
     */
    protected static boolean updateRatConfig(int iRat){
        if (checkRatConfig(iRat) == true) {
            forceUpdateRatConfig(iRat);
            return true;
        } else {
            logd("updateRatConfig: illedge rat config " + String.valueOf(iRat));
            return false;
        }
    }
