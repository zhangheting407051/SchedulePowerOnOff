...
public class UiccCardApplication {
    ...
    public void update (IccCardApplicationStatus as, Context c, CommandsInterface ci) {
        synchronized (mLock) {
            ...
            if (DBG) log("mPersoSubState: " + mPersoSubState +
    ...
    public boolean getIccFdnAvailable() {
        ...
        IccRecords.IccServiceStatus iccSerStatus = mIccRecords.getSIMServiceStatus(
                IccRecords.IccService.FDN);
    ...
    public void queryIccNetworkLock(int category, Message onComplete) {
        ...
        switch(category) {
            ...
            case CAT_SIM:
                mCi.queryNetworkLock(category,
                        mHandler.obtainMessage(EVENT_QUERY_NETWORK_LOCK_DONE, onComplete));
                break;
            default:
    ...
    public void setIccNetworkLockEnabled(int category, int lockop, String password,
        ...
        switch(lockop) {
            ...
            case OP_UNLOCK:
                mCi.setNetworkLock(category, lockop, password, data_imsi, gid1, gid2,
                        mHandler.obtainMessage(EVENT_CHANGE_NETWORK_LOCK_DONE, onComplete));
                break;
            default:
