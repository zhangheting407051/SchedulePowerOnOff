...
public class ImsPhoneCall extends Call {
    ...
    public void switchWith(ImsPhoneCall that) {
        ...
        synchronized (ImsPhoneCall.class) {
            ...
            /// M: ALPS02602246, the mOwner of the temp ImsPhoneCall
            /// might be access by other thread while takeOver. Should assign an initial
            /// value to avoid null reference exception. @{
            ImsPhoneCall tmp = new ImsPhoneCall(mOwner, ImsPhoneCall.CONTEXT_UNKNOWN);
            /// @}
            tmp.takeOver(this);
            this.takeOver(that);
            that.takeOver(tmp);
