...
public class AdnRecordCache extends Handler implements IccConstants {
    ...
    public synchronized void updateAdnByIndex(int efid, AdnRecord adn, int recordIndex, String pin2,
        ...
        if (!mUsimPhoneBookManager.checkEmailCapacityFree(recordIndex, adn.mEmails)) {
            sendErrorResponse(
                    response,
        ...
        if (!mUsimPhoneBookManager.checkSneCapacityFree(recordIndex, adn.sne)) {
            sendErrorResponse(
                    response,
                    "drop the sne for the limitation of the SIM card",
                    RILConstants.SNE_SIZE_LIMIT);
            return;
        }
        ...
        if (efid == IccConstants.EF_ADN || efid == IccConstants.EF_PBR ||
            ...
            try {
                int mResult = mUsimPhoneBookManager.updateSneByAdnIndex(adn.sne, recordIndex);
                if (UsimPhoneBookManager.USIM_ERROR_CAPACITY_FULL == mResult) {
                    sendErrorResponse(
                            response,
                            "drop the SNE for the limitation of the SIM card",
                            RILConstants.SNE_SIZE_LIMIT);
                } else if (UsimPhoneBookManager.USIM_ERROR_STRING_TOOLONG == mResult) {
                    sendErrorResponse(
                            response,
                            "the sne string is too long",
                            RILConstants.SNE_NAME_TOOLONG);
                    Rlog.e(LOG_TAG, "haman, by index sne too long");
                }
                ...
                int success = mUsimPhoneBookManager.
                        updateEmailsByAdnIndex(adn.mEmails, recordIndex);
                if (UsimPhoneBookManager.USIM_ERROR_CAPACITY_FULL == success) {
                    sendErrorResponse(
                            response,
                            "drop the email for the limitation of the SIM card",
                            RILConstants.EMAIL_SIZE_LIMIT);
                } else if (UsimPhoneBookManager.USIM_ERROR_STRING_TOOLONG == success) {
                    sendErrorResponse(
                            response,
                            "the email string is too long",
                            RILConstants.EMAIL_NAME_TOOLONG);
                    Rlog.e(LOG_TAG, "haman, by index email too long");
                } else if (UsimPhoneBookManager.USIM_ERROR_OTHERS == success) {
                    sendErrorResponse(
                            response,
                            "Unkown error occurs when update email",
                            RILConstants.GENERIC_FAILURE);
                    Rlog.e(LOG_TAG, "Unkown error occurs when update email");
                } else {
    ...
    public synchronized int updateAdnBySearch(int efid, AdnRecord oldAdn, AdnRecord newAdn,
        ...
        */
        ...
        if (!mUsimPhoneBookManager.checkSneCapacityFree(index, newAdn.sne)) {
            sendErrorResponse(
                    response,
                    "drop the sne for the limitation of the SIM card",
                    RILConstants.SNE_SIZE_LIMIT);
        ...
        if (efid == EF_ADN || efid == EF_PBR || efid == 0x4F3A || efid == 0x4F3B ||
            ...
            int mResult = mUsimPhoneBookManager.updateSneByAdnIndex(newAdn.sne, index);
            if (UsimPhoneBookManager.USIM_ERROR_CAPACITY_FULL == mResult) {
                sendErrorResponse(
                        response,
                        "drop the SNE for the limitation of the SIM card",
                ...
                synchronized (mLock) {
                    if (mLocked) {
                        ...
                        if (ar.exception == null && adn != null) {
                            ...
                            logd("mAdnLikeFiles changed index:" + index + ",adn:" + adn + "  efid:"
                                    + efid);

                            if (null != mAdnLikeFiles && null != mAdnLikeFiles.get(efid)) {
                                if (efid == 0x4F3B && mFh instanceof CsimFileHandler) {
                                    index -= 250;
                                }
