...
public class IccSmsInterfaceManager {
    ...
    public boolean copyMessageToIccEf(String callingPackage, int status, byte[] pdu, byte[] smsc) {
        ...
        synchronized(mLock) {
            ...
            if (PhoneConstants.PHONE_TYPE_GSM == mPhone.getPhoneType()) {
                ...
                // MTK-START: To make sure the app passed PDU which is created by newFromParcel
                // can be process by writeSmsToRuim.
    ...
    public void sendMultipartTextWithEncodingType(String callingPackage, String destAddr,
        ...
        if (parts.size() > 1 && parts.size() < 10 && !SmsMessage.hasEmsSupport()) {
            for (int i = 0; i < parts.size(); i++) {
                ...
                mDispatcher.sendTextWithEncodingType(destAddr, scAddr, singlePart,
                        encodingType, singleSentIntent, singleDeliveryIntent,
                        null/*messageUri*/, callingPackage,
                        persistMessageForNonDefaultSmsApp);
            }
            return;
    ...
    public void sendMultipartTextWithExtraParams(String callingPackage, String destAddr,
        ...
        if (parts.size() > 1 && parts.size() < 10 && !SmsMessage.hasEmsSupport()) {
            for (int i = 0; i < parts.size(); i++) {
                ...
                mDispatcher.sendTextWithExtraParams(destAddr, scAddr, singlePart,
                        extraParams, singleSentIntent, singleDeliveryIntent,
                        null/*messageUri*/, callingPackage,
                        persistMessageForNonDefaultSmsApp);
            }
            return;
     ...
     */
    protected SimSmsInsertStatus writeTextMessageToRuim(String address,
            List<String> text, int status, long timestamp) {
        ...
        for (int i = 0; i < text.size(); ++i) {
            ...
            if (pdu != null) {
                Message response = mHandler.obtainMessage(EVENT_INSERT_TEXT_MESSAGE_TO_ICC_DONE);
