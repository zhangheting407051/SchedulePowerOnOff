...
public class VoiceInteractionManagerService extends SystemService {
    ...
    //added logs to debug complex issues to check execution flow
    ...
    public VoiceInteractionManagerService(Context context) {
        super(context);
        if (LOGS) {
        Slog.i(TAG, "service instantiated");
        }
        mContext = context;
    ...
    public void onStart() {
        if (LOGS) {
        Slog.i(TAG, "service onStart starts..");
        }
    ...
    public void onBootPhase(int phase) {
        if (LOGS) {
        Slog.i(TAG, "service onStart starts.." + phase);
        }
    ...
    public void onStartUser(int userHandle) {
        if (LOGS) {
        Slog.i(TAG, "service onStartUser" + userHandle);
}
    ...
    public void onUnlockUser(int userHandle) {
        if (LOGS) {
        Slog.i(TAG, "service onUnlockUser starts.." + userHandle);
        }
        mServiceStub.initForUser(userHandle);
    ...
    public void onSwitchUser(int userHandle) {
        if (LOGS) {
        Slog.i(TAG, "service onSwitchUser" + userHandle);
        }
    ...
    class VoiceInteractionManagerServiceStub extends IVoiceInteractionManagerService.Stub {
            ...
            try {
                mImpl.showSessionLocked(options,
                        ...
                        new IVoiceInteractionSessionShowCallback.Stub() {
                            ...
                            public void onShown() {
                                if (LOGS) {
                                Slog.i(TAG, "startLocalVoiceInteraction");
                                }
        ...
        public void stopLocalVoiceInteraction(IBinder callingActivity) {
            if (LOGS) {
            Slog.i(TAG, "stopLocalVoiceInteraction starts.." + mImpl);
            }
            if (mImpl == null) return;
        ...
        public boolean supportsLocalVoiceInteraction() {
            if (LOGS) {
            Slog.i(TAG, "supportsLocalVoiceInteraction starts.." + mImpl);
            }
        ...
        private boolean shouldEnableService(Resources res) {
            if (LOGS) {
            Slog.i(TAG, "shouldEnableService starts");
            }
        ...
        public void systemRunning(boolean safeMode) {
            if (LOGS) {
            Slog.i(TAG, "systemRunning starts");
            }
            mSafeMode = safeMode;
        ...
        void switchImplementationIfNeededLocked(boolean force) {
            if (LOGS) {
            Slog.i(TAG, "switchImplementationIfNeededLocked starts, force flag : " + force);
            }
            if (!mSafeMode) {
        ...
        VoiceInteractionServiceInfo findAvailInteractor(int userHandle, String packageName) {
            ...
            if (LOGS) {
            Slog.i(TAG, "findAvailInteractor starts.. " + userHandle + packageName);
            }
            if (numAvailable == 0) {
        ...
        ComponentName getCurInteractor(int userHandle) {
            ...
            if (TextUtils.isEmpty(curInteractor)) {
                if (LOGS) {
                Slog.i(TAG, "getCurInteractor curInteractor NULL");
                }
                return null;
            }
        ...
        public void showSession(IVoiceInteractionService service, Bundle args, int flags) {
            if (LOGS) {
            Slog.i(TAG, "showSession starts.." + flags);
            }
            synchronized (this) {
        ...
        public boolean deliverNewSession(IBinder token, IVoiceInteractionSession session,
            ...
            if (LOGS) {
                Slog.i(TAG, "deliverNewSession starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
                    throw new SecurityException(
        ...
        public boolean showSessionFromSession(IBinder token, Bundle sessionArgs, int flags) {
            if (LOGS) {
            Slog.i(TAG, "showSessionFromSession starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public boolean hideSessionFromSession(IBinder token) {
            if (LOGS) {
            Slog.i(TAG, "hideSessionFromSession starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public int startVoiceActivity(IBinder token, Intent intent, String resolvedType) {
            if (LOGS) {
            Slog.i(TAG, "startVoiceActivity starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public void setKeepAwake(IBinder token, boolean keepAwake) {
            if (LOGS) {
            Slog.i(TAG, "setKeepAwake starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public void closeSystemDialogs(IBinder token) {
            if (LOGS) {
            Slog.i(TAG, "closeSystemDialogs starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public void setDisabledShowContext(int flags) {
            if (LOGS) {
            Slog.i(TAG, "setDisabledShowContext starts.." + flags);
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public int getDisabledShowContext() {
            if (LOGS) {
            Slog.i(TAG, "getDisabledShowContext starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
        ...
        public int getUserDisabledShowContext() {
            if (LOGS) {
            Slog.i(TAG, "getUserDisabledShowContext starts..");
            }
            synchronized (this) {
                if (mImpl == null) {
                    Slog.w(TAG,
