...
class VoiceInteractionManagerServiceImpl implements VoiceInteractionSessionConnection.Callback {
    ...
    final BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (LOGS) {
                Slog.i(TAG, "onReceive starts.." + intent.getAction());
            }
    ...
    final ServiceConnection mConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            if (LOGS) {
                Slog.i(TAG, "onServiceConnected starts..");
            }
            synchronized (mLock) {
        ...
        public void onServiceDisconnected(ComponentName name) {
            if (LOGS) {
                Slog.i(TAG, "onServiceDisconnected starts..");
            }
            mService = null;
        }
    };
    ...
    VoiceInteractionManagerServiceImpl(Context context, Handler handler, Object lock,
        ...
        if (LOGS) {
            Slog.i(TAG, "VoiceInteractionManagerServiceImpl starts..");
        }
        try {
    ...
    public boolean showSessionLocked(Bundle args, int flags,
        ...
        if (LOGS) {
            Slog.i(TAG, "showSessionLocked statrts..mActiveSession " + mActiveSession);
        }
    ...
    public boolean hideSessionLocked() {
        if (LOGS) {
            Slog.i(TAG, "hideSessionLocked starts");
        }
        if (mActiveSession != null) {
    ...
    public boolean deliverNewSessionLocked(IBinder token,
        ...
        if (LOGS) {
            Slog.i(TAG, "deliverNewSessionLocked ends success..");
        }
        return true;
    }
    ...
    public int startVoiceActivityLocked(int callingPid, int callingUid, IBinder token,
        ...
        if (LOGS) {
            Slog.i(TAG, "startVoiceActivityLocked starts..");
        }
        try {
            if (mActiveSession == null || token != mActiveSession.mToken) {
    ...
    public void setKeepAwakeLocked(IBinder token, boolean keepAwake) {
        if (LOGS) {
            Slog.i(TAG, "setKeepAwakeLocked starts..");
        }
        try {
            if (mActiveSession == null || token != mActiveSession.mToken) {
    ...
    public void launchVoiceAssistFromKeyguard() {
        if (LOGS) {
            Slog.i(TAG, "launchVoiceAssistFromKeyguard starts..");
        }
    ...
    void shutdownLocked() {
        if (LOGS) {
            Slog.i(TAG, "shutdownLocked starts..");
        }
    ...
    void notifySoundModelsChangedLocked() {
        if (LOGS) {
            Slog.i(TAG, "notifySoundModelsChangedLocked starts..");
        }
